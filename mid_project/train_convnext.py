...
def main(): 
    ...
    for x,data in enumerate(stf):
        ...
        # validate the model
            ...
            # calculate the accuracy of the validation predictions
                ...
                print("Test :Epoch[{:0>3}/{:0>3}] Iteration[{:0>3}/{:0>3}] Loss: {:.4f} Acc:{:.2%} AUC:{:.2}\n Sensitivity of benign:{:.2} Sensitivity of malignant:{:.2} Specificity of benign:{:.2} Specificity of malignant:{:.2}".format(
                epoch, MAX_EPOCH, i+1, len(train_loader), loss_val, val_acc,Auc,sensitivity[0],sensitivity[1],specificity[0],specificity[1]))
        ...
        c.save("training_progress of folder for convnext {:0>2}.png".format(x+1))
    ...
    print('The max validation accuracy is: {:.2%}, auc is {:.2%}.\n'.format(sum(folder_acc)/k,sum(folder_auc)/k))
    plt.cla()
    plt.clf()
    ...
    ax.get_figure().savefig('Final_acc for convnext.png')
    plt.cla()
    plt.clf()
    ...
    ax.get_figure().savefig('Final_Auc for convnext.png')
    plt.cla()
    plt.clf()
...
if __name__ == '__main__':
    ...
    all_data=MyDataset(data_dir,indexes,None)
    indexes_labels=[]
    for i in indexes:
        indexes_labels.append(all_data.__getitem__(i)[1])
    indexes_labels=np.array(indexes_labels)
    set_seed(seed)
